    ...
    function(data) {
      switch(content_class){
        case "tvshows":
          for (i = 0; i < data.Items.length; i++) {
            console.log(i + " " + data.Items[i].Name);
            document.getElementById("items").innerHTML += "<img id='" + data.Items[i].Id + "' src='" + whole_url + "/Items/" + data.Items[i].Id + "/Images/Primary' width=180 height=270 onClick='seasons(this.id);'</img>";
          }
        break;
        case "movies":
          for (i = 0; i < data.Items.length; i++) {
            console.log(i + " " + data.Items[i].Name);
            document.getElementById("items").innerHTML += "<img id='" + data.Items[i].Id + "' src='" + whole_url + "/Items/" + data.Items[i].Id + "/Images/Primary' width=180 height=270 onClick='play(content_id_global);'</img>";
          }
        break;
        case "music":
        break;
...
function seasons(content_id){
  ...
  function(data) {
    for (i = 0; i < data.Items.length; i++) {
      console.log(i + " " + data.Items[i].Name);
      document.getElementById("seasons").innerHTML += "<img id='" + data.Items[i].Id + "' src='" + whole_url + "/Items/" + data.Items[i].SeriesId + "/Images/Primary' width=180 height=270 onClick='episodes(this.id);'</img>";
    }
  },
  function(xhr) {
    console.error(xhr);
...
function episodes(content_id){
  ...
  for (i = 0; i < data.Items.length; i++) {
    console.log(i + " " + data.Items[i].Name);
    document.getElementById("episodes").innerHTML += "<img id='" + data.Items[i].Id + "' src='" + whole_url + "/Items/" + data.Items[i].Id + "/Images/Primary' width=400 height=225 onClick='play(this.id);'</img>";
  }
  },
  function(xhr) {
  console.error(xhr);
